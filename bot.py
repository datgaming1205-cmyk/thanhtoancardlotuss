...
# Lệnh chỉnh sửa số tiền TOP thủ công
@bot.command(name="setuptopcard")
@commands.has_permissions(administrator=True)
async def setuptopcard(ctx, user: discord.Member, amount: int):
...
# Lệnh trừ tiền TOP thủ công
@bot.command(name="deltopcard")
@commands.has_permissions(administrator=True)
async def deltopcard(ctx, user: discord.Member, amount: int):
    """Trừ số tiền nạp CARD từ tổng của người dùng"""
    conn = sqlite3.connect('orders.db')
    c = conn.cursor()
    c.execute("SELECT total_spent FROM leaderboard WHERE user_id = ?", (user.id,))
    res = c.fetchone()
    
    if res:
        new_amount = max(0, res[0] - amount)
        conn.execute("UPDATE leaderboard SET total_spent = ? WHERE user_id = ?", (new_amount, user.id))
        conn.commit()
        await ctx.send(f"✅ Đã trừ **{amount:,} VND** từ {user.mention}. Tổng mới: **{new_amount:,} VND**.")
    else:
        await ctx.send(f"❌ Người dùng {user.mention} chưa có dữ liệu trên bảng xếp hạng CARD.")
